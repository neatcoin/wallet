...
const findPackagesApps = require('../../apps/scripts/findPackages.cjs');
...
function createWebpack (context, mode = 'production') {
  ...
  const aliasApps = findPackagesApps().reduce((alias, { dir, name }) => {
    alias[name] = path.resolve(context, `../../apps/packages/${dir}/src`);
